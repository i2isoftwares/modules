  ...
  void _resetTap(BuildContext context) async{
    if(passwordController.text.isNotEmpty && confirmPasswordController.text.isNotEmpty){
      if(passwordController.text == confirmPasswordController.text){
        UserRegistrationSignin response = await postSaveRegPassword(widget.data['token'], passwordController.text, context);
...
//          }
//        }else{
//          ResetPassword response = await postResetPassword(widget.data['email'], passwordController.text, context);
//          Utils.showToastMsg(response.message, context);
//          Future.delayed(Duration(seconds: 1));
//          if(response.status){
//            Navigator.of(context).popUntil((route) {
...
//        }
        ...
        Utils.showToastMsg('Password does not match', context);
      }
    }else{
      Utils.showToastMsg('Please enter password', context);
    }
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: customAppBar(
        ...
        centerTitle: false,
        title: Text(
          widget.data.containsKey('isFromRegister') ? 'New User Registration' : 'Reset Password',
                    ...
                    children: <Widget>[
                      ...
                      Container(
                        ...
                        child: Column(
                          children: <Widget>[
                            CustomTextField(
                              placeholder: widget.data.isNotEmpty ? "Password" : "New Password",
                              obscureText: true,
                            ...
                            SizedBox(height: 24),
                            CustomTextField(
                              placeholder: widget.data.isNotEmpty ? "Confirm Password" : "Confirm New Password",
                              obscureText: true,
                            ...
                            SizedBox(height: 24),
                            CustomButton(
                              buttonText: widget.data.isNotEmpty ? 'Submit' : 'Reset',
                              onPressed: () {
